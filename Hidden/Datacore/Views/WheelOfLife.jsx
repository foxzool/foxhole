...
function IdealWheelOfLifeChart() {
  ...
  const idealRatings = fields.map((label) => {
    ...
    if (!currentYearNote) {
      throw new Error(
        `Missing yearly note, please ensure the note [[My Calendar/My Yearly Notes/${year}]] can be found.`
      );
    }
    if (rating === undefined) {
      throw new Error(
...
function PeriodicReviewWheelOfLifeChart() {
  ...
  if (fields.length !== labels.length || fields.length !== areaColors.length) {
    return (
      <dc.Callout title={"Error"} type={"warning"}>
        Configuration file error: fields, labels, and colors must have the same
        length.
      </dc.Callout>
    );
  }
